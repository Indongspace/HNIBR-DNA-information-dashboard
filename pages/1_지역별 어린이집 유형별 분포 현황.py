# pages/1_지역별 어린이집 유형별 분포 현황.py
import pandas as pd
import streamlit as st
import altair as alt
from pathlib import Path

st.set_page_config(page_title="지역별 어린이집 유형별 분포(개수·비율·합계)", layout="wide")
st.title("지역별 어린이집 유형별 분포")

# -----------------------------
# 데이터 경로 입력 (CSV 권장)
# -----------------------------
DEFAULT_DATA = "data/지역별 어린이집 유형별 분포 현황.csv"
data_path = st.sidebar.text_input("데이터명: 지역별 어린이집 유형별 분포 현황", DEFAULT_DATA)
st.sidebar.caption("행정구역별, 어린이집 유형별 분포 현황입니다.")
...
axis_xcnt = alt.Axis(title="어린이집 개수(총합 길이)", labelFontSize=label_font)
    ...
    base_bars = (
        ...
        .encode(
            ...
            tooltip=[
                ...
                alt.Tooltip("유형:N"),
                alt.Tooltip("sum(개수):Q", title="유형별 개수", format=",.0f"),
